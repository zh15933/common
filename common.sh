...
sed -i '/dahuilang1/d; /helloworld/d; /passwall/d; /OpenClash/d' "feeds.conf.default"
cat feeds.conf.default|awk '!/^#/'|awk '!/^$/'|awk '!a[$1" "$2]++{print}' >uniq.conf
mv -f uniq.conf feeds.conf.default

# 这里增加了源,要对应的删除/etc/opkg/distfeeds.conf插件源
cat >>"feeds.conf.default" <<-EOF
src-git dahuilang1 https://github.com/shidahuilang/openwrt-package.git;${SOURCE}
EOF
...
if [[ `find "${App_path}" -type d -name "zh_Hans" |grep -c "zh_Hans"` -gt '20' ]]; then
  echo "src-git dahuilang2 https://github.com/shidahuilang/openwrt-package.git;Theme2" >> "feeds.conf.default"
else
  echo "src-git dahuilang2 https://github.com/shidahuilang/openwrt-package.git;Theme1" >> "feeds.conf.default"
fi
...
t=(${z//,/ })
for x in ${t[@]}; do \
  find . -type d -name "${x}" |grep -v 'dahuilang\|freifunk' |xargs -i rm -rf {}; \
done
